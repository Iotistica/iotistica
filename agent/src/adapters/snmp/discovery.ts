...
export class SNMPDiscoveryPlugin extends BaseDiscoveryPlugin {
	...
	async discover(options?: SNMPDiscoveryOptions): Promise<DiscoveredDevice[]> {
		...
		const discoveryTargets =
			this.configManager?.getDiscoveryTargets?.("snmp") || [];

		if (discoveryTargets.length === 0) {
			this.logger?.debugSync(
				"No SNMP discovery targets configured (need community without dataPoints)",
				{
		...
		this.logger?.debugSync(
			`Starting SNMP discovery (${discoveryTargets.length} targets)`,
			{
				component: (LogComponents.discovery + "] [" + this.protocol) as any,
				protocol: this.protocol,
				phase: "discovery",
		...
		this.logger?.debugSync(
			`Scanning ${nonGatewayHosts.length} IP addresses for SNMP devices`,
			{
				component: (LogComponents.discovery + "] [" + this.protocol) as any,
					...
					limit(async () => {
						try {
							...
							if (deviceInfo) {
								...
								const fingerprint = generateSNMPFingerprint(
									ip,
									deviceInfo.sysObjectID || deviceInfo.sysDescr,
								);
		...
		this.logger?.debugSync(
			`SNMP discovery complete: ${discovered.length} devices found`,
			{
				component: (LogComponents.discovery + "] [" + this.protocol) as any,
	...
	private getDefaultIPRanges(): string[] {
		...
		this.logger?.warnSync(
			"getDefaultIPRanges() called but SNMP_IP_RANGES not set - network scan prevented",
			{
				component: (LogComponents.discovery + "] [" + this.protocol) as any,
		...
		for (const range of ranges) {
			if (range.includes("/")) {
				...
				try {
					...
					this.logger?.debugSync(
						`Resolved hostname ${range} to ${resolved.address}`,
						{
							component: (LogComponents.discovery +
								"] [" +
								this.protocol) as any,
	 ...
	 */
	private parseSysDescr(sysDescr: string): {
		manufacturer?: string;
		modelNumber?: string;
	} {
