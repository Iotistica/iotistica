...
| Field | Value |
|---|---|
| **Endpoint** | `mqtt.googleapis.com` (or your replacement broker host) |
| **Port** | `8883` (default) |
| **Client ID** | Full GCP device path: `projects/<project>/locations/<region>/registries/<registry>/devices/<device>` |
| **JWT / Password** | The JWT token generated in Step 4 |
| **Root CA** | (Optional) Google's root CA PEM — paste if TLS verification fails |
