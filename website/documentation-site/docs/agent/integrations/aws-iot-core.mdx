...
| Field | Value |
|---|---|
| **Endpoint** | Device data endpoint from Step 4 |
| **Device ID** | Thing name from Step 1 (used as MQTT client ID and in topic) |
| **Client Certificate** | Paste the full contents of `device.pem.crt` |
| **Private Key** | Paste the full contents of `device.private.key` |
| **Root CA** | Paste the full contents of the Amazon Root CA 1 file (optional but recommended) |
| **Port** | `8883` (default) |
