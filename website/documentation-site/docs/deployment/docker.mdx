...
**6. Seed the MQTT admin user** (required for agent connections — see [MQTT Users](#mqtt-users)).
...
After the stack starts for the first time, seed the admin MQTT user to match the `MQTT_USERNAME` / `MQTT_PASSWORD` values in `.env.self`:

```bash
docker compose -f docker-compose.self.yml exec api \
  node -e "
    const { Pool } = require('pg');
    const bcrypt = require('bcryptjs');
    const pool = new Pool({ connectionString: process.env.DATABASE_URL });
    const hash = bcrypt.hashSync(process.env.MQTT_PASSWORD, 10);
    pool.query(
      \`INSERT INTO mqtt_users (username, password_hash, is_superuser)
       VALUES (\$1, \$2, true)
       ON CONFLICT (username) DO UPDATE SET password_hash = \$2\`,
      [process.env.MQTT_USERNAME, hash]
    ).then(() => { console.log('done'); pool.end(); });
  "
```

Without this step, all MQTT connections will be rejected with 403.
