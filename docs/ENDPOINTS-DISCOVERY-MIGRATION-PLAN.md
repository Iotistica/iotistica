...
- **Modbus**: Scans `slaveRange` to find responding devices ΓåÆ applies profile dataPoints (user-configured)
- **OPC-UA**: Browses server `endpointUrl` ΓåÆ discovers nodes/variables automatically  
- **SNMP**: Queries `host` with `community` string ΓåÆ discovers OIDs automatically
...
async function migrateProtocolsToEndpoints(deviceUuid: string): Promise<MigrationReport> {
  ...
  console.log(`Γ£à Migrated ${report.endpointsCreated} discovery targets for device ${deviceUuid}`);
  return report;
}
...
public getDiscoveryTargets(protocol: string): any[] {
  ...
  if (targets.length === 0 && this.cloudConfig?.protocols?.[protocol]) {
    this.logger?.warnSync(
      `ΓÜá∩╕Å DEPRECATED: Using protocols.${protocol} for discovery. ` +
...
**Discovery Behavior**:
- **Modbus**: Scans `slaveRange`, finds responding slaves ΓåÆ creates endpoint rows with profile-defined dataPoints
- **OPC-UA**: Browses `endpointUrl`, discovers nodes ΓåÆ creates endpoint row with discovered nodeIds as dataPoints
- **SNMP**: Queries `host`, discovers OIDs ΓåÆ creates endpoint row with discovered OIDs as dataPoint
- `connection.slaveRange` present ΓåÆ Discovery target (scan this range)
- `connection.slaveId` present ΓåÆ Operational device (poll this specific slave)
...
- Run migration script to convert `protocols{}` ΓåÆ `endpoints[]`
...
- Γ£à No schema changes (existing types support discovery)
- Γ£à Structural detection (slaveRange vs slaveId, endpointUrl with/without dataPoints, etc.)
- Γ£à Global scheduling (intervals.discovery already configured)
- Γ£à Single source of truth (`endpoints` table)
- Γ£à Dashboard integration (filter by connection structure)
...
- Γ£à No new fields needed (slaveRange already exists)
- Γ£à No schema changes (existing types support discovery)
- Γ£à Structural detection (slaveRange vs slaveId)
- Γ£à Global scheduling (intervals.discovery already configured)
- Γ£à Single source of truth (`endpoints[]`)
- Γ£à Dashboard integration (filter by connection structure)

**Total changes**: Just update `AgentConfig.getDiscoveryTargets()` to read from `endpoints[]` instead of `protocols{}`.

**Migration**: Map `protocols.modbus.connections[]` ΓåÆ `endpoints[]` with `slaveRange` field.
