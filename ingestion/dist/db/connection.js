"use strict";
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.pool = void 0;
exports.getPoolStats = getPoolStats;
exports.query = query;
exports.getClient = getClient;
exports.transaction = transaction;
exports.testConnection = testConnection;
exports.close = close;
const pg_1 = require("pg");
const logger_1 = __importDefault(require("../utils/logger"));
function parseIntEnv(name, fallback) {
    const raw = process.env[name];
    if (!raw)
        return fallback;
    const parsed = parseInt(raw, 10);
    return Number.isFinite(parsed) ? parsed : fallback;
}
function clamp(value, min, max) {
    return Math.min(max, Math.max(min, value));
}
function readClampedIntEnv(name, fallback, min, max) {
    const requestedValue = parseIntEnv(name, fallback);
    const boundedValue = clamp(requestedValue, min, max);
    if (requestedValue !== boundedValue) {
        logger_1.default.warn(`${name} out of safe bounds; clamping value`, {
            requestedValue,
            boundedValue,
            minAllowed: min,
            maxAllowed: max,
        });
    }
    return boundedValue;
}
const boundedPoolSize = readClampedIntEnv('DB_POOL_SIZE', 50, 5, 100);
const idleTimeoutMillis = readClampedIntEnv('DB_IDLE_TIMEOUT_MS', 30000, 1000, 300000);
const connectionTimeoutMillis = readClampedIntEnv('DB_CONNECTION_TIMEOUT_MS', 30000, 1000, 120000);
const statementTimeout = readClampedIntEnv('DB_STATEMENT_TIMEOUT_MS', 60000, 1000, 600000);
const dbApplicationName = process.env.DB_APPLICATION_NAME || 'iotistic-ingestion';
const dbConfig = {
    host: process.env.DB_HOST || 'localhost',
    port: parseInt(process.env.DB_PORT || '5432', 10),
    database: process.env.DB_NAME || 'iotistic',
    user: process.env.DB_USER || 'postgres',
    password: process.env.DB_PASSWORD || 'postgres',
    max: boundedPoolSize,
    idleTimeoutMillis,
    connectionTimeoutMillis,
    statementTimeout,
    application_name: dbApplicationName,
    allowExitOnIdle: false,
    keepAlive: true,
    keepAliveInitialDelayMillis: 10000,
    ssl: process.env.DB_SSL === 'false' ? false : {
        rejectUnauthorized: process.env.DB_SSL_REJECT_UNAUTHORIZED === 'false' ? false : true,
    },
};
exports.pool = new pg_1.Pool(dbConfig);
function getPoolStats() {
    const totalClients = exports.pool.totalCount;
    const idleClients = exports.pool.idleCount;
    const waitingClients = exports.pool.waitingCount;
    const activeClients = Math.max(0, totalClients - idleClients);
    const configuredMax = boundedPoolSize;
    const denom = configuredMax > 0 ? configuredMax : Math.max(1, totalClients);
    const saturationPct = Number(((activeClients / denom) * 100).toFixed(1));
    return {
        total: totalClients,
        idle: idleClients,
        active: activeClients,
        waiting: waitingClients,
        saturationPct,
        configuredMax,
    };
}
exports.pool.on('error', (err) => {
    logger_1.default.error('Unexpected error on idle database client', err);
});
const TRANSIENT_PG_CODES = new Set(['57P01', '57P02', '57P03', '08000', '08006', '08001', '08004']);
const TRANSIENT_NODE_CODES = new Set(['ECONNRESET', 'ECONNREFUSED', 'ENOTFOUND', 'ETIMEDOUT', 'EAI_AGAIN']);
function isTransientError(error) {
    if (!(error instanceof Error))
        return false;
    const code = error.code;
    if (!code)
        return false;
    return TRANSIENT_PG_CODES.has(code) || TRANSIENT_NODE_CODES.has(code);
}
function isWriteQuery(text) {
    return /^\s*(INSERT|UPDATE|DELETE|MERGE)\b/i.test(text);
}
function pgErrorContext(error) {
    if (!(error instanceof Error))
        return {};
    const e = error;
    return {
        pgCode: e.code,
        pgSeverity: e.severity,
        pgDetail: e.detail,
        pgHint: e.hint,
        pgTable: e.table,
        pgConstraint: e.constraint,
        pgColumn: e.column,
        message: e.message,
    };
}
async function query(text, params) {
    const maxAttempts = 3;
    const isWrite = isWriteQuery(text);
    const maxQueryPreview = 1000;
    const textPreview = text.length > maxQueryPreview ? `${text.slice(0, maxQueryPreview)}... [truncated ${text.length - maxQueryPreview} chars]` : text;
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            return await exports.pool.query(text, params);
        }
        catch (error) {
            if (isTransientError(error) && attempt < maxAttempts) {
                const delayMs = attempt * 500;
                logger_1.default.warn('Transient DB error, retrying query...', {
                    attempt,
                    maxAttempts,
                    retryInMs: delayMs,
                    isWrite,
                    ...pgErrorContext(error),
                });
                await new Promise((resolve) => setTimeout(resolve, delayMs));
                continue;
            }
            const poolStats = getPoolStats();
            const transientExhausted = isTransientError(error);
            if (isWrite || !transientExhausted) {
                logger_1.default.error(transientExhausted ? 'Write query failed after all retries — data may not have been persisted' : 'Query failed with non-transient error', {
                    isWrite,
                    attemptsUsed: attempt,
                    text: textPreview,
                    paramsCount: Array.isArray(params) ? params.length : 0,
                    dbPool: poolStats,
                    ...pgErrorContext(error),
                });
            }
            else {
                logger_1.default.warn('Read query failed after all retries', {
                    attemptsUsed: attempt,
                    text: textPreview,
                    paramsCount: Array.isArray(params) ? params.length : 0,
                    dbPool: poolStats,
                    ...pgErrorContext(error),
                });
            }
            throw error;
        }
    }
    throw new Error('Unexpected unreachable query retry state');
}
async function getClient() {
    try {
        return await exports.pool.connect();
    }
    catch (err) {
        logger_1.default.warn('Failed to acquire database client from pool', err);
        throw new Error('DB connection temporarily unavailable');
    }
}
async function transaction(callback) {
    const client = await getClient();
    try {
        await client.query('BEGIN');
        const result = await callback(client);
        await client.query('COMMIT');
        return result;
    }
    catch (error) {
        try {
            await client.query('ROLLBACK');
        }
        catch (rollbackError) {
            logger_1.default.warn('Failed to rollback transaction (connection may be closed)', rollbackError);
        }
        throw error;
    }
    finally {
        try {
            client.release();
        }
        catch (releaseError) {
            logger_1.default.warn('Failed to release client', releaseError);
        }
    }
}
async function testConnection() {
    try {
        logger_1.default.info('Testing database connection with config:', {
            host: dbConfig.host,
            port: dbConfig.port,
            database: dbConfig.database,
            user: dbConfig.user,
            applicationName: dbApplicationName,
            poolMax: boundedPoolSize,
            idleTimeoutMillis,
            connectionTimeoutMillis,
            statementTimeout,
        });
        const result = await query('SELECT NOW() as now');
        logger_1.default.info(`Database connected successfully at ${String(result.rows[0].now)}`);
        return true;
    }
    catch (error) {
        logger_1.default.error('Database connection failed:', error);
        if (error instanceof Error) {
            logger_1.default.error('Connection error details:', {
                message: error.message,
                code: error.code,
                host: dbConfig.host,
                port: dbConfig.port,
            });
        }
        return false;
    }
}
async function close() {
    await exports.pool.end();
    logger_1.default.info('Database connections closed');
}
//# sourceMappingURL=connection.js.map