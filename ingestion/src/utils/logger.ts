...
function writeLog(loggerInstance: PinoLogger, level: LogLevel, first: unknown, second?: unknown): void {
  ...
  switch (level) {
    case 'info':
      if (meta && msg) loggerInstance.info(meta, msg);
      else if (meta) loggerInstance.info(meta);
      else if (msg) loggerInstance.info(msg);
      return;
    case 'warn':
      if (meta && msg) loggerInstance.warn(meta, msg);
      else if (meta) loggerInstance.warn(meta);
      else if (msg) loggerInstance.warn(msg);
      return;
    case 'error':
      if (meta && msg) loggerInstance.error(meta, msg);
      else if (meta) loggerInstance.error(meta);
      else if (msg) loggerInstance.error(msg);
      return;
    case 'debug':
      if (meta && msg) loggerInstance.debug(meta, msg);
      else if (meta) loggerInstance.debug(meta);
      else if (msg) loggerInstance.debug(msg);
      return;
  }
}
