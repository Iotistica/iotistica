-- Migration 015: Rename readings.device_uuid -> agent_uuid
--
-- The raw readings hypertable stored the gateway/agent UUID under the name
-- device_uuid, which conflicted with the per-device uuid5 stored in
-- extra->>'device_uuid'.  Migrations 012-014 cleaned up the views; this
-- migration renames the raw column so the whole stack is consistent:
--
--   readings.agent_uuid              = gateway / agent UUID  (renamed)
--   readings.extra->>'device_uuid'   = per-device uuid5      (unchanged)
--   readings.extra->>'endpoint_uuid' = protocol endpoint     (unchanged)
--
-- Requires dropping and recreating:
--   - 4 continuous aggregates  (reference old column in GROUP BY / SELECT)
--   - 3 materialized views     (reference r.device_uuid via readings alias)
--   - 1 regular view           (device_anomaly_summary)
--   - 2 index renames           (idx_readings_device_time, readings_pkey implicit)
--
-- Data is preserved throughout.  Continuous aggregates are recreated WITH NO
-- DATA; their refresh policies are restored and a manual REFRESH is run to
-- re-populate existing aggregate buckets.
--

BEGIN;

-- ============================================================================
-- 1. DROP DEPENDENT OBJECTS (top-down: views that depend on readings column)
-- ============================================================================

-- Regular view
DROP VIEW IF EXISTS device_anomaly_summary;

-- Materialized views (drop indexes first to avoid lock contention)
...
-- Continuous aggregates (CASCADE also removes their refresh policies)
...
-- ============================================================================
-- 2. RENAME COLUMN ON THE HYPERTABLE
-- ============================================================================

ALTER TABLE readings RENAME COLUMN device_uuid TO agent_uuid;

-- Rename indexes whose names reference device_uuid for clarity
ALTER INDEX IF EXISTS idx_readings_device_time RENAME TO idx_readings_agent_time;

-- ============================================================================
-- 3. RECREATE CONTINUOUS AGGREGATES  (use agent_uuid directly, no alias)
-- ============================================================================

-- readings_1m: 1-minute buckets
...
-- readings_hourly: hourly with LAST/FIRST for last-known-value queries
...
SELECT DISTINCT ON (r.agent_uuid, r.extra->>'device_name', r.metric_name)
    r.agent_uuid,
...
ORDER BY r.agent_uuid, r.extra->>'device_name', r.metric_name, r.time DESC;

CREATE UNIQUE INDEX idx_latest_readings_unique   ON latest_readings (agent_uuid, device_name, metric_name);
CREATE INDEX        idx_latest_readings_device   ON latest_readings (device_name);
CREATE INDEX        idx_latest_readings_protocol ON latest_readings (protocol);
CREATE INDEX        idx_latest_readings_quality  ON latest_readings (quality);
CREATE INDEX        idx_latest_readings_agent    ON latest_readings (agent_uuid);
...
-- ============================================================================
-- 5. RECREATE REGULAR VIEW
-- ============================================================================

CREATE VIEW device_anomaly_summary AS
SELECT
    agent_uuid,
    extra->>'deviceName'                        AS device_name,
    AVG(anomaly_score)
        FILTER (WHERE time > NOW() - INTERVAL '24 hours')       AS avg_anomaly_24h,
    MAX(anomaly_score)
        FILTER (WHERE time > NOW() - INTERVAL '24 hours')       AS max_anomaly_24h,
    COUNT(*)
        FILTER (WHERE time > NOW() - INTERVAL '24 hours'
                  AND anomaly_score > 0.7)                      AS high_anomaly_count_24h,
    (SELECT r2.anomaly_score
     FROM readings r2
     WHERE r2.agent_uuid = r.agent_uuid
       AND r2.anomaly_score IS NOT NULL
     ORDER BY r2.time DESC LIMIT 1)                             AS latest_anomaly_score,
    (SELECT r2.time
     FROM readings r2
     WHERE r2.agent_uuid = r.agent_uuid
       AND r2.anomaly_score IS NOT NULL
     ORDER BY r2.time DESC LIMIT 1)                             AS latest_scored_time,
    (SELECT r2.metric_name
     ...
     ORDER BY AVG(r2.anomaly_score) DESC LIMIT 1)               AS most_anomalous_metric,
    COUNT(DISTINCT metric_name)
        FILTER (WHERE anomaly_score IS NOT NULL
                  AND time > NOW() - INTERVAL '24 hours')       AS monitored_metrics_count
FROM readings r
WHERE anomaly_score IS NOT NULL
  AND time > NOW() - INTERVAL '7 days'
GROUP BY agent_uuid, extra->>'deviceName';

-- ============================================================================
-- 6. RESTORE REFRESH POLICIES
-- ============================================================================
...
-- ============================================================================
-- 7. BACKFILL CONTINUOUS AGGREGATES  (must run outside a transaction block)
--    CALL refresh_continuous_aggregate cannot run inside BEGIN/COMMIT.
-- ============================================================================

CALL refresh_continuous_aggregate('readings_1m',      NOW() - INTERVAL '7 days', NOW());
CALL refresh_continuous_aggregate('readings_1h',      NOW() - INTERVAL '7 days', NOW());
CALL refresh_continuous_aggregate('readings_hourly',  NOW() - INTERVAL '7 days', NOW());
CALL refresh_continuous_aggregate('readings_daily',   NOW() - INTERVAL '7 days', NOW());

-- ============================================================================
-- 8. REFRESH MATERIALIZED VIEWS  (non-concurrent since metric_catalog /
--    endpoint_devices have no unique index required for CONCURRENTLY)
-- ============================================================================
