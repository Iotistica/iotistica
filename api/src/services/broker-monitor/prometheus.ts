/**
 * Minimal Prometheus renderer for MQTT/Broker Monitor.
    ...
    for (const metric of METRICS) {
      ...
      switch (metric.name) {
        ...
        case 'mqtt_monitor_sampled_messages_total': {
          const total = Object.values(this.snapshot.sampledByReason).reduce((a, b) => a + b, 0);
          lines.push(formatMetric(metric.name, total));
          for (const [reason, count] of Object.entries(this.snapshot.sampledByReason)) {
            lines.push(formatMetric(metric.name, count, { reason }));
          }
          break;
        }
      }
