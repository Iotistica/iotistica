/**
 * MQTT Database Service (Broker Monitor)
...
export class MQTTDatabaseService {
  ...
  async saveSchemaHistoryBatch(items: Array<{
    ...
    try {
      ...
      if (valuesToInsert.length > 0) {
        const values = valuesToInsert.map((_, idx) => {
          const base = idx * 5;
  ...
  async getTopicRecentActivity(topicOrId: string, windowMinutes: number = 15): Promise<{
    ...
    // Determine if it's a UUID or topic name
    const isUuid = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(topicOrId);

    let topicName = topicOrId;
    if (isUuid) {
      const topicRecord = await this.pool.query(
        'SELECT topic FROM mqtt_topics WHERE topic_id = $1',
        [topicOrId]
      );
      if (topicRecord.rows.length === 0) return null;
      topicName = topicRecord.rows[0].topic;
    }

    const result = await this.pool.query(`
      SELECT timestamp, message_count
      FROM mqtt_topic_metrics
      WHERE topic = $1 AND timestamp >= NOW() - INTERVAL '${windowMinutes} minutes'
      ORDER BY timestamp ASC
    `, [topicName]);

    if (result.rows.length === 0) {
      return { topic: topicName, messageCount: 0, messageRate: 0, dataPoints: [] };
    }
    ...
    const messageCount = dataPoints.length > 1
      ? dataPoints[dataPoints.length - 1].count - dataPoints[0].count
      : 0;

    const windowSec = windowMinutes * 60;
    const messageRate = windowSec > 0 ? Math.round((messageCount / windowSec) * 100) / 100 : 0;

    return { topic: topicName, messageCount, messageRate, dataPoints };
  }

  /**
   * Load initial state from database (topics + latest broker stats)
   */
  async loadInitialState(): Promise<{
    ...
    const [topics, stats] = await Promise.all([
      this.getTopics({ limit: 10000 }),
      this.getLatestBrokerStats(),
    ]);
    return { topics, stats };
  }
}
